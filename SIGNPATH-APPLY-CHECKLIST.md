# SignPath Foundation application checklist

Before applying:

- [x] Public source repository created
- [x] OSI-approved MIT license added
- [x] Build instructions documented
- [x] Privacy policy documented
- [x] Security policy documented
- [x] Code signing policy documented
- [x] LiveKit dependency/version documented
- [x] LiveKit SDK release archive pinned and SHA-256 checked in CI
- [ ] GitHub Actions public build succeeds
- [ ] GitHub account MFA/2FA enabled for maintainers and signing approvers
- [ ] Project has a normal public release/tag
- [ ] SignPath Foundation application submitted
- [ ] SignPath project, artifact configuration and signing policy created
- [ ] SignPath GitHub integration configured
- [ ] Manual signing approval tested

The private backend, dashboard and server credentials are not part of this client repository and must remain private.
